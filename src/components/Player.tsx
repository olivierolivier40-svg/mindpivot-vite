...
const MusicOffIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19V6l12-3v13M9 19c0 1.1.895 2 2 2s2-.9 2-2M9 19c0-1.1.895-2 2-2s2 .9 2 2m9-11l-6-1.5" /><path strokeLinecap="round" strokeLinejoin="round" d="M3 3l18 18" /></svg>;
...
export const Player = ({ ritual: initialRitual, onComplete, onBack, sessions, onCheckForNewBadges, soundSettings, setSoundSettings, checkinData, onShowInfo }: PlayerProps) => {
  ...
  const [timeLeft, setTimeLeft] = useState(initialRitual.dureeSec);
  ...
  const [showDonut, setShowDonut] = useState(!!(initialRitual.donut && initialRitual.donut !== 'off'));
  const [phaseTime, setPhaseTime] = useState({current: 0, total: 0});
  const [phaseProgress, setPhaseProgress] = useState(0);
  const [linearPhaseProgress, setLinearPhaseProgress] = useState(0); 
  ...
  const [freqDuration, setFreqDuration] = useState<number>(1); 
  const [showFreqInfo, setShowFreqInfo] = useState(false);

  const [showBentoOptions, setShowBentoOptions] = useState(initialRitual.playerType === 'bento');
  ...
  useEffect(() => {
      setRitual(initialRitual); setTimeLeft(initialRitual.dureeSec); setInstruction(initialRitual.instructions || '');
      ...
      setIsRunning(false); setIsPaused(false); setIsComplete(false); setNewlyUnlockedBadgeId(null);
      setAudioProgress(0); setRitualPhaseIndex(0); setSagesseAgreementIndex(0);
  ...
  }, [initialRitual, soundSettings.enabled, t]);

  useEffect(() => {
    if (ritual.playerType === 'sagesse-minute') generateNewQuote();
    if (ritual.playerType === 'couleur' && ritual.data?.colors) setColorVirtue(ritual.data.colors[currentColor.toUpperCase()] || '');
  ...
  useEffect(() => {
      ...
      if (isRunning && !isPaused) {
          ...
          if (!oscRef.current) {
              ...
              osc.type = 'sine'; osc.frequency.value = freqHz; gain.gain.value = 0;
              ...
              if (gainRef.current) gainRef.current.gain.setTargetAtTime(soundSettings.volume, ctx.currentTime, 0.1);
          }
      } else {
          if (gainRef.current && oscRef.current) {
              ...
              if (ctx) {
                  ...
                  setTimeout(() => { if (oscRef.current) { try { oscRef.current.stop(); oscRef.current.disconnect(); } catch(e) { console.error(e); } oscRef.current = null; } }, 200);
              }
          }
      }
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => setIsAuroraTheme(document.documentElement.classList.contains('theme-aurora')));
  ...
  const tick = useCallback((timestamp: number) => {
        ...
        if (elapsedSec >= ritual.dureeSec) { setTimeLeft(0); stop(true); return; }
        setTimeLeft(ritual.dureeSec - Math.floor(elapsedSec));
        let currentPhaseProgress = 0;
        if (ritual.id === 'rit.coherence_bento' && elapsedSec >= 60 && bentoPhase === 'short' && !bentoReadyForPhase2) setBentoReadyForPhase2(true);
        ...
        if (isRunning && isBreathingPhase && (showDonut || (currentPhaseData && currentPhaseData.type === 'respiration'))) {
            ...
            } else { protocol = protocolMap[ritual.donut as keyof typeof protocolMap]; }

            if (protocol) {
                const totalCycleTime = protocol.reduce((sum, step) => sum + step.s, 0);
                if (totalCycleTime > 0) {
                    const timeInCycle = timeForBreathingCalc % totalCycleTime;
                    let cumulativeTime = 0; let currentPhaseIdx = -1;
                    for (let i = 0; i < protocol.length; i++) {
                        cumulativeTime += protocol[i].s;
                        if (timeInCycle < cumulativeTime) { currentPhaseIdx = i; break; }
                    }
                    if (currentPhaseIdx === -1) currentPhaseIdx = 0;
                    if(currentPhaseIdx !== -1) {
                        const currentPhase = protocol[currentPhaseIdx];
                        const phaseStartTime = protocol.slice(0, currentPhaseIdx).reduce((sum, p) => sum + p.s, 0);
                        const progressInPhaseRaw = Math.min(1, Math.max(0, (timeInCycle - phaseStartTime) / currentPhase.s));
                        setLinearPhaseProgress(progressInPhaseRaw);
                        const phaseName = t(currentPhase.n).toLowerCase();
                        if (phaseName.startsWith('expire') || phaseName.startsWith('exhale')) currentPhaseProgress = 1 - progressInPhaseRaw;
                        else if (phaseName.includes('pleins') || phaseName.includes('full') || phaseName.includes('bloque')) currentPhaseProgress = 1;
                        else if (phaseName.includes('vides') || phaseName.includes('empty') || phaseName.includes('pause')) currentPhaseProgress = 0;
                        else currentPhaseProgress = progressInPhaseRaw;
                        ...
                        if (ritual.playerType === 'bento') {
                            ...
                            const mantraKey = currentPhase.n.startsWith('breathe_in') ? currentMantraSet[bentoPhase].inhale : currentMantraSet[bentoPhase].exhale;
                        ...
                        setDonutLabel(newLabel); setBreathPhaseIndex(currentPhaseIdx);
                        if (currentPhaseIdx !== lastPhaseIndexRef.current) { 
                            const bipPhaseName = t(currentPhase.n).toLowerCase();
                            if (bipPhaseName.startsWith('inspire') || bipPhaseName.startsWith('inhale')) playBip(432, 'sine');
                            else if (bipPhaseName.startsWith('expire') || bipPhaseName.startsWith('exhale')) playBip(288, 'sine');
                            else if (bipPhaseName.includes('retiens') || bipPhaseName.includes('hold') || bipPhaseName.includes('bloque')) playBip(350, 'triangle');
                            lastPhaseIndexRef.current = currentPhaseIdx; 
                        }
                    }
                }
            }
        ...
        if (RITUAL_INSTRUCTIONS[ritual.id]) {
            ...
            if (currentInstruction && dynamicInstruction !== currentInstruction.text) { setDynamicInstruction(currentInstruction.text); playPhaseTone(); }
            const reversedInstructions = [...instructions].reverse();
            const currentIdxInReversed = reversedInstructions.findIndex(i => elapsedSec >= i.time);
            if (currentIdxInReversed !== -1) {
                const actualIndex = instructions.length - 1 - currentIdxInReversed;
                ...
                if (phaseDuration > 0) currentPhaseProgress = Math.min(1, (elapsedSec - phaseStartTime) / phaseDuration);
            }
        }
        if (ritual.playerType === 'slideshow') {
            let cumulativeDuration = 0; let newIdx = 0;
            for(let i=0; i < ritual.data.images.length; i++) {
                cumulativeDuration += ritual.data.images[i].duration;
                if (elapsedSec < cumulativeDuration) { newIdx = i; break; }
            }
            if (slideshowIndex !== newIdx) { setSlideshowIndex(newIdx); playPhaseTone(); }
            let phaseStartTime = 0; for (let i = 0; i < newIdx; i++) phaseStartTime += ritual.data.images[i].duration;
            const phaseDuration = ritual.data.images[newIdx].duration;
            if (phaseDuration > 0) currentPhaseProgress = Math.min(1, (elapsedSec - phaseStartTime) / phaseDuration);
        }
        if (ritual.playerType === 'organe-smile') {
            const organDuration = ritual.dureeSec / ORGANES.length;
            const newIdx = Math.floor(elapsedSec / organDuration);
            if(newIdx < ORGANES.length && currentOrgan.index !== newIdx) { setCurrentOrgan({ index: newIdx, ...ORGANES[newIdx] }); playPhaseTone(); }
            if(organDuration > 0) currentPhaseProgress = (elapsedSec % organDuration) / organDuration;
        }
        if (ritual.playerType === 'sagesse-minute') {
            const phaseDuration = ritual.dureeSec / 4; const newIdx = Math.floor(elapsedSec / phaseDuration);
            if (newIdx !== sagesseAgreementIndex) { setSagesseAgreementIndex(newIdx); playPhaseTone(); }
            if(phaseDuration > 0) currentPhaseProgress = (elapsedSec % phaseDuration) / phaseDuration;
        }
        if (ritual.playerType === 'phased-ritual') {
            let cumulativeDuration = 0; let newPhaseIdx = -1;
            for(let i=0; i < ritual.data.phases.length; i++) {
                if (elapsedSec < cumulativeDuration + ritual.data.phases[i].duration) { newPhaseIdx = i; break; }
                cumulativeDuration += ritual.data.phases[i].duration;
            }
             if (newPhaseIdx === -1) newPhaseIdx = ritual.data.phases.length -1;
            if (ritualPhaseIndex !== newPhaseIdx) { playPhaseTone(); setRitualPhaseIndex(newPhaseIdx); }
            const phaseDuration = ritual.data.phases[newPhaseIdx].duration;
            const timeInPhase = elapsedSec - cumulativeDuration;
  ...
  const restartRitual = () => {
      ...
      setAudioProgress(0); setPhaseProgress(0); setLinearPhaseProgress(0); setBreathPhaseIndex(0);
  ...
  const handleAIGeneratedIntention = async () => {
      setIsGeneratingIntention(true); setCustomIntention(null);
      try {
          const summary = Object.entries(checkinData).filter(([, v]) => v !== 0).map(([k, v]) => `${t(`label_${k}_title`)}: ${t(LABELS[k as keyof typeof LABELS][v+2])}`).join(', ');
          const prompt = `Basé sur ce bilan matinal : "${summary || 'Aucun bilan fourni'}", propose une seule intention de journée, laïque et introspective (15 mots max). Phrase affirmative à la première personne. Ne retourne que l'intention.`;
          const intention = await generateGeminiText(prompt); setCustomIntention(intention.trim());
      } catch (error) { console.error(error); setCustomIntention("Désolé, une erreur est survenue."); } finally { setIsGeneratingIntention(false); }
  };
  const toggleMute = () => setSoundSettings(prev => ({...prev, enabled: !prev.enabled}));
      ...
      <div className="w-full max-w-md min-h-full flex flex-col items-center justify-center gap-4 px-4 pt-10 pb-48 animate-fade-in">
          ...
          <div className="flex flex-wrap justify-center gap-3">
              {ritual.data?.frequencies.map((f: any) => (
                  <button key={f.hz} onClick={() => { setFreqHz(f.hz); setFreqLabel(f.label); setFreqDesc(f.desc); }} className={`w-14 h-14 rounded-full flex items-center justify-center font-bold text-sm transition-transform hover:scale-110 border-2 ${freqHz === f.hz ? 'border-accent bg-accent/20 text-accent' : 'border-fg/20 bg-fg/5 text-muted'}`}>{f.hz}</button>
              ))}
          </div>
          <div className="text-center p-3 bg-white/5 rounded-lg border border-white/10 min-h-[5em] w-full flex flex-col justify-center">
              <p className="font-bold text-lg mb-1">{t(freqLabel)}</p><p className="text-sm text-muted whitespace-pre-line leading-tight">{t(freqDesc)}</p>
          </div>
          <div className="flex flex-col items-center gap-2 w-full">
              <span className="text-sm text-muted font-semibold">{t('player_freq_select_duration')}</span>
              <div className="flex gap-2">{[1, 3, 5].map(d => (<Button key={d} variant={freqDuration === d ? 'primary' : 'secondary'} size="small" onClick={() => { setFreqDuration(d); setRitual(prev => ({...prev, dureeSec: d * 60})); setTimeLeft(d * 60); }}>{d} {t('unit_min')}</Button>))}</div>
          </div>
      </div>
  );
  ...
  const renderPreStartContent = () => {
    if (ritual.preStartSteps && preStartStepIndex < ritual.preStartSteps.length) {
        ...
        return <div className="w-full h-full flex items-center justify-center p-4"><div className="relative z-10 text-center animate-fade-in-short font-display"><div className="mb-6 text-lg" dangerouslySetInnerHTML={{ __html: t(step.text).replace(/\n/g, '<br />') }} /></div></div>;
    }
  ...
  const renderActiveContent = () => {
    ...
    if (ritual.id === 'rit.box_4_4_4_4') return <SquareBreathAnimation label={donutLabel} progress={linearPhaseProgress} phase={breathPhaseIndex} />;
    ...
    if (ritual.playerType === 'phased-ritual' && ritual.data.phases[ritualPhaseIndex]) {
        ...
        if (phase.type === 'texte') {
          const isTextOnly = ritual.data.phases.every((p: any) => p.type === 'texte');
          const txtClasses = isTextOnly ? "text-xl lg:text-2xl font-semibold leading-relaxed" : "text-lg";
          return <div className={`${txtClasses} whitespace-pre-line animate-fade-in p-4 w-full max-h-full overflow-y-auto`} dangerouslySetInnerHTML={{__html: t(phase.instruction).replace(/\n/g, '<br /><br />')}}></div>;
        }
        if (phase.type === 'image') return <div className="flex-1 w-full min-h-0 flex items-center justify-center animate-fade-in"><img src={phase.imageUrl} alt={t('visual_instruction_alt')} referrerPolicy="no-referrer" className={`max-h-full max-w-full rounded-lg mx-auto ${phase.backgroundFill ? 'object-cover w-full h-full' : 'object-contain'}`} /></div>;
        if (phase.type === 'video') return <div className="flex-1 w-full min-h-0 flex items-center justify-center animate-fade-in"><video src={phase.videoUrl} autoPlay loop muted={!isVideoSoundOn} playsInline className="max-h-full max-w-full rounded-lg mx-auto object-contain" /></div>;
        if (phase.type === 'animation') return <div className="w-full h-full flex flex-col items-center justify-center gap-4 animate-fade-in"><div className="flex-1 w-full min-h-0 flex items-center justify-center"><EyeMovementAnimation slow={ritual.id === 'rit.recyclage_eclair'} /></div></div>;
        if (phase.type === 'fixation') return <div className="w-12 h-12 rounded-full bg-black animate-pulse"></div>;
        if (phase.type === 'respiration') {
          if (ritual.id === 'rit.synchro_gamma') return <SquareBreathAnimation label={donutLabel} progress={linearPhaseProgress} phase={breathPhaseIndex} />;
          return <RingDonut label={donutLabel} ratio={phaseProgress} sessionProgressRatio={(phaseTime.current / phaseTime.total)} />;
        }
    }
    ...
    if (ritual.playerType === 'slideshow' && ritual.data?.images) {
        const curImg = ritual.data.images[slideshowIndex];
        return <div className={`w-full h-full relative overflow-hidden ${isImmersive ? '' : 'rounded-lg'}`}><div key={slideshowIndex} className="absolute inset-0 w-full h-full bg-cover bg-center animate-fade-in-slow" style={{ backgroundImage: `url(${curImg.url})` }} /><div className={`absolute inset-0 w-full h-full ${isImmersive ? 'bg-black/70' : 'bg-black/60'}`}></div><div className="w-full h-full flex flex-col items-center justify-center relative z-10 p-8 text-center text-white"><p key={slideshowIndex + '-caption'} className={`${isImmersive ? 'text-4xl' : 'text-3xl'} font-semibold [text-shadow:_0_2px_8px_rgb(0_0_0_/_90%)] animate-fade-in`}>{t(curImg.caption)}</p></div></div>;
    }
    ...
    if (ritual.playerType === 'sagesse-minute' && currentQuote) return <SagesseMinutePlayer quote={currentQuote} phaseIndex={sagesseAgreementIndex} />;
    if (ritual.playerType === 'organe-smile' && currentOrgan.videoUrl) return <div className="w-full h-full flex flex-col items-center justify-center p-4"><div className="flex-1 w-full min-h-0 flex items-center justify-center rounded-lg overflow-hidden"><video key={currentOrgan.videoUrl} src={currentOrgan.videoUrl} autoPlay loop muted={!isVideoSoundOn} playsInline className="w-full h-full object-contain" /></div><div className="mt-4 p-3 text-center text-xl font-semibold animate-fade-in"><p>{t('player_organe_smile_instruction', { organName: t(currentOrgan.name) })}</p></div></div>;
    ...
    if (ritual.playerType === 'frequency-healing') return <div className="w-full h-full flex flex-col items-center justify-center relative gap-8"><FrequencyVisualizer frequency={freqHz} isRunning={isRunning && !isPaused} color="#22d3ee" /><div className="relative z-10 flex flex-col items-center animate-fade-in"><div className="text-6xl font-bold mb-2 tracking-tighter" style={{ textShadow: '0 0 20px rgba(34,211,238,0.3)' }}>{freqHz} <span className="text-2xl font-normal">Hz</span></div><button onClick={() => setShowFreqInfo(true)} className="mt-6 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors text-sm flex items-center gap-2 border border-white/10"><InfoIcon /> {t('player_know_more')}</button></div></div>;
    const fallbackInst = t(instruction) || (RITUAL_INSTRUCTIONS[ritual.id]?.[0]?.text ? t(RITUAL_INSTRUCTIONS[ritual.id]?.[0]?.text) : '');
    if(fallbackInst) return <p className="text-lg text-muted" dangerouslySetInnerHTML={{ __html: fallbackInst.replace(/\n/g, '<br />') }}></p>;
    return <p className="text-lg text-muted">{t('follow_instructions')}</p>;
  };
  
  const renderBentoOptions = () => (<div className="flex flex-col items-center justify-center gap-4 animate-fade-in"><h3 className="text-lg font-bold">{t('player_bento_personalize')}</h3><p className="text-sm text-muted">{t('player_bento_mantra_theme')}</p><div className="flex flex-wrap justify-center gap-2">{Object.entries(BENTO_MANTRAS).map(([key, value]) => (<Button key={key} variant={bentoTheme === key ? 'primary' : 'secondary'} onClick={() => setBentoTheme(key)}>{t(value.name)}</Button>))}</div></div>);
  const renderNadiShodhanaOptions = () => (<div className="flex flex-col items-center justify-center gap-4 animate-fade-in"><h3 className="text-lg font-bold">{t('player_nadi_choose_goal')}</h3><div className="flex gap-4"><Button variant={breathingDirection === 'apaisant' ? 'primary' : 'secondary'} onClick={() => setBreathingDirection('apaisant')}>{t('player_nadi_apaisant')}</Button><Button variant={breathingDirection === 'dynamisant' ? 'primary' : 'secondary'} onClick={() => setBreathingDirection('dynamisant')}>{t('player_nadi_dynamisant')}</Button></div><p className="text-sm text-muted max-w-xs">{breathingDirection === 'apaisant' ? t('player_nadi_apaisant_desc') : t('player_nadi_dynamisant_desc')}</p><Button variant="ghost" onClick={() => setShowMudraModal(true)}>{t('player_nadi_finger_position')}</Button></div>);
  const renderCouleurOptions = () => (<div className="flex flex-col items-center justify-center gap-4 animate-fade-in"><h3 className="text-lg font-bold">{t('player_couleur_choose_color')}</h3><div className="flex flex-wrap justify-center gap-3">{Object.entries(ritual.data.colors).map(([color, virtue]) => (<button key={color} onClick={() => setCurrentColor(color)} className="w-12 h-12 rounded-full transition-transform hover:scale-110" style={{ backgroundColor: color, border: currentColor === color ? '4px solid white' : '4px solid transparent' }} aria-label={t(virtue as string)}></button>))}</div><p className="font-bold text-lg" style={{ color: currentColor }}>{t(colorVirtue)}</p></div>);
  const renderIntentionOptions = () => (<div className="flex flex-col items-center justify-center gap-6 animate-fade-in text-center"><div className="min-h-[6em] flex items-center justify-center">{isGeneratingIntention ? (<p className="text-muted animate-pulse">{t('loading_analysis')}</p>) : (<p className="text-xl font-semibold italic">{customIntention ? `"${customIntention}"` : t(ritual.instructions || '')}</p>)}</div><div className="flex items-center justify-center gap-4 w-full"><div className="flex flex-col items-center gap-2"><Button onClick={() => setShowIntentionsModal(true)} variant="secondary" className="!p-0 w-14 h-14 flex items-center justify-center rounded-full"><ListIcon /></Button><span className="text-xs text-muted">{t('intention_button_examples')}</span></div><div className="flex flex-col items-center gap-2"><Button onClick={handleRandomIntention} variant="secondary" className="!p-0 w-14 h-14 flex items-center justify-center rounded-full"><DiceIcon /></Button><span className="text-xs text-muted">{t('intention_button_random')}</span></div><div className="flex flex-col items-center gap-2"><Button onClick={handleAIGeneratedIntention} disabled={isGeneratingIntention} variant="secondary" className="!p-0 w-14 h-14 flex items-center justify-center rounded-full"><SparkleIcon /></Button><span className="text-xs text-muted">{t('intention_button_ai')}</span></div></div></div>);
  ...
  const handlePreStartNext = () => { if (ritual.preStartSteps && preStartStepIndex < ritual.preStartSteps.length - 1) setPreStartStepIndex(prev => prev + 1); else start(); };
  if (isPreStart && ritual.preStartSteps) return <Modal show={true} title={`${ritual.modal.icon} ${t(ritual.label)} - ${'Préparation'}`} onClose={onBack} hideHeaderCloseButton={false} preStartNext={handlePreStartNext}><div className="w-full h-64 bg-cover bg-center rounded-lg mb-4" style={{ backgroundImage: `url(${ritual.preStartSteps[preStartStepIndex].backgroundUrl})` }} /><div className="text-center">{renderPreStartContent()}</div></Modal>;

  const hasVideo = ritual.playerType === 'organe-smile' || (ritual.data?.phases && ritual.data.phases.some((p: any) => p.type === 'video'));
  let secInst: string | null = null;
  if (isRunning && !isPaused && ritual.playerType === 'phased-ritual' && ritual.data.phases[ritualPhaseIndex]) {
    const phase = ritual.data.phases[ritualPhaseIndex];
    if ((phase.type === 'image' || phase.type === 'video' || phase.type === 'animation') && phase.instruction) secInst = t(phase.instruction);
  }
  const phRem = Math.max(0, Math.round(phaseTime.total - phaseTime.current));
  const showPhTimer = ritual.playerType === 'phased-ritual' && phaseTime.total > 0;
