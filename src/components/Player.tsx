...
return (
            <div className={`w-full flex flex-col items-center ${isImmersive ? 'min-h-full pb-60 justify-start pt-20' : 'h-full justify-start pt-4'} p-4`}>
  ...
  const renderActiveContent = () => {
    ...
    if (ritual.playerType === 'slideshow' && ritual.data?.images) {
        ...
        return (
            <div className={`w-full h-full relative overflow-hidden ${isImmersive ? '' : 'rounded-lg'}`}>
                {/* Background Image - No Dark Filter on top anymore */}
                <div key={slideshowIndex} className="absolute inset-0 w-full h-full bg-cover bg-center animate-fade-in-slow" style={{ backgroundImage: `url(${curImg.url})` }} />
                
                {/* Text Container */}
                <div className="absolute inset-0 flex flex-col items-center justify-end pb-32 z-10 pointer-events-none p-4">
                    <p key={slideshowIndex + '-caption'} className="text-xl md:text-2xl text-white bg-black/60 px-6 py-4 rounded-xl backdrop-blur-md shadow-lg animate-fade-in font-medium leading-relaxed max-w-4xl">
                        {t(curImg.caption)}
                    </p>
                </div>
            </div>
    ...
    if (ritual.playerType === 'frequency-healing') return <div className="w-full h-full flex flex-col items-center justify-center relative gap-8"><FrequencyVisualizer frequency={freqHz} isRunning={isRunning && !isPaused} color="#22d3ee" /><div className="relative z-10 flex flex-col items-center animate-fade-in"><div className="text-6xl font-bold mb-2 tracking-tighter" style={{ textShadow: '0 0 20px rgba(34,211,238,0.3)' }}>{freqHz} <span className="text-2xl font-normal">Hz</span></div><Button onClick={() => setShowFreqInfo(true)} variant="info" size="large" className="mt-6 w-48"><InfoIcon /> {t('player_know_more')}</Button></div></div>;
...
<main className={isImmersive ? "w-full flex-1 flex flex-col items-center min-h-0 overflow-y-auto" : "w-full flex-1 flex flex-col items-center justify-start pt-4 min-h-0 relative p-4 overflow-y-auto"}>{isPreStart ? renderPreStartContent() : renderActiveContent()}{hasVideo && isRunning && !isPreStart && (<button onClick={() => setIsVideoSoundOn(p => !p)} className="absolute top-6 right-6 z-30 w-10 h-10 flex items-center justify-center rounded-full bg-black/40 text-white hover:bg-black/60 transition-colors" title={t('player_video_music')} aria-label={t('player_toggle_music')}>{isVideoSoundOn ? <MusicOnIcon /> : <MusicOffIcon />}</button>)}</main>        {isImmersive ? (
